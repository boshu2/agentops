...
4. Inspect the packet before editing:
...
5. Confirm that the packet role is `implement`, the provider is `codex`, the
   ...
   change directory to the exact absolute `workspace` printed by `inspect`;
   the pool session directory is transport scaffolding, not the subject.
...
When the experiment has been handled, atomically emit one adapter response:
