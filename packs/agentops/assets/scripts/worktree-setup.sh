...
#
# Vendored from examples/gastown/packs/gastown/assets/scripts/worktree-setup.sh
# (the proven GC worktree-isolation glue the refinery pre_start relies on). Kept
# byte-for-byte so the AgentOps refinery inherits the exact branch-namespacing,
# stage/restore, and origin/HEAD-refresh behavior gastown's worker uses.
# TODO(gvkj6-finalize): re-sync if gastown's upstream copy changes post-rip.
#
