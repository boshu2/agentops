...
**AgentOps is the navigator for stochastic agentic work** — the deterministic apparatus (the map, the route, the position signal, the corpus, the loop) that drives an agent from a *goal* to a *verified done*, and compounds the context that makes the next drive better. You don't script the route; you set the destination and the navigator drives it, trusting the environment (the map, the gates, the real test that runs) over the stochastic agent. **AgentOps is complete as a standalone product**: its own validation membrane (`ao validate` / `ao gate` emitting claims), navigator loop, corpus/flywheel, and CDLC are enough to drive the work without Mount Olympus. **Mount Olympus is an optional extension** that consumes AgentOps outputs and upgrades selected claims into binding cross-family verdicts; AgentOps never depends on MTO.
...
> **Autonomous goal → verified done.** Give AgentOps a goal carrying an acceptance contract, and the navigator drives a stochastic agent to a *verified* done — shape intent → slice → route → build → gate (AgentOps validation membrane; optionally MTO for binding cross-family verdicts) → recover on failure (re-scope with the failure as new acceptance, or pull the andon) → ratchet the learning — **without a human in each step.**
...
The destination is reached by AgentOps itself: a complete standalone product with its own validation membrane, navigator loop, corpus/flywheel, and CDLC. The trust boundary inside standalone AgentOps is the membrane between stochastic work and explicit evidence-backed claims. Mount Olympus is not part of the core dependency chain; it is the optional high-assurance extension that consumes those claims when a binding cross-family verdict is required.

- **AgentOps is the standalone producing / sensing / mining factory.** It drives the work, emits deterministic evidence to the provenance ledger (**SENSOR**), gates that evidence through its validation membrane (`ao validate` / `ao gate`), and mines the evidence into proposed improvements (**ASSAY**). It produces and it *claims* — those claims are first-class product outputs.
- **Mount Olympus is the optional trusted-verdict extension.** `olympusd` can consume AgentOps claims and write binding terminal verdicts — ≥2 model families, no self-approval, fail-closed. It judges only when the operator extends AgentOps into MTO; it never becomes a prerequisite for standalone AgentOps.

Standalone AgentOps runs the core loop. When extended into MTO, the extension exchanges with AgentOps on **two axes**:

1. **The work axis (per artifact):** AgentOps's SENSOR emits evidence-backed claims → Mount Olympus's GATE consumes selected claims and writes the binding verdict → **the verdict returns to AgentOps as a first-class provenance-ledger event** (`claude-code-review` verdicts are first-class ledger events). Evidence out, verdict back — and the verdict is now new sensor data. The extended loop closes *through the ledger*.
2. **The recurrence axis (per pattern):** AgentOps's ASSAY mines the accumulated evidence, claims, and any returned verdicts → proposes an apparatus change (a skill, a gate, a rule) → Mount Olympus can gate **the proposal** or hand back recurrence signal when high-assurance ratification is required → the ratified change ratchets into the apparatus.

The trust comes from **separation of duties**, not from a core product dependency: stochastic workers do not self-approve; AgentOps forces them through evidence, gates, claims, and provenance. *"Trust the environment, not the agent"* — at the factory altitude. MTO is the optional stronger environment for binding verdicts. Directive 16's route stays an AgentOps route: milestone 1 is the sensor/ledger, milestone 3 is AgentOps's validation membrane with an optional MTO upgrade, and milestone 4 is self-improvement from evidence, claims, and returned verdicts when present. The destination — *autonomous goal → verified done* — is AgentOps running unattended end-to-end; extending into MTO adds binding cross-family assurance.
...
3. **The gate** — "done" must be *verified*, not merely asserted: AgentOps's own validation membrane (`ao validate` / `ao gate`) emits evidence-backed claims at the significant-action boundary; MTO can optionally upgrade selected claims into binding cross-family verdicts, fail-closed. Deterministic ground-truth (the ledger, the gates, the real test) is the windshield that catches a hallucinated done. *Status: AgentOps membrane live; MTO extension in flight; provenance windshield poured.*
