...
This is the executable fitness contract for the product intent in
[`PRODUCT.md`](PRODUCT.md). Directives describe desired outcomes; each
`Progress` paragraph must describe the current default build honestly. When a
directive conflicts with executable behavior or a declared contract, the lower
claim is stale.

## Destination

> **Autonomous goal → verified done.** Given a goal with testable acceptance,
> the operating loop shapes, slices, builds, verifies, repairs, and lands the
> work with an evidence-bound independent verdict.

The first wedge is autonomous code validation, not a general agent control
plane. The verification membrane is the proven product. Corpus retrieval,
knowledge compounding, and broader unattended execution remain experimental
until controlled measurements show task-level improvement.

Work earns priority when it advances the operating loop, validation/release
health, tracker or provenance truth, measured context value, or runtime proof
for that core route. The [component map](docs/architecture/component-map.md)
owns the keep/defer/trim boundary.

## Fitness principles

<!-- agentops:claim:AOP-CLAIM-GOALS-DREAM-VALIDATED -->

- No evidence-bound independent verdict means not done. Deterministic checks
  establish facts; a fresh context judges semantic acceptance.
- Workers and model runtimes are replaceable; acceptance and evidence must
  survive them. Cross-model quorum is an optional assurance posture.
- Workspace-local `.agents/` material is fallible runtime state. A learning is
  promoted only when it changes a future-consumed plan, skill, test, contract,
  or gate.
- A new user should reach one validated flow in under five minutes.
- Product promises without a proof path, code-only vanity metrics, and hidden
  quarantined regressions do not count as progress.
...
Every advertised installer needs a smoke proof that its installed skill set is
structurally usable. A new installer is incomplete until it joins that matrix.

**Progress:** `tests/install/test-install-smoke.sh` provides structural and
dry-run coverage, and `install-e2e.yml` exercises clean installs. The current
suite is **not green**: its command assertions still expect archive/omitted
surfaces (`flywheel` and `inject`) from a default-spine binary, and the Gemini
image verifier also fails. The repair is to test the five advertised installer
paths against commands actually shipped by their selected profile.
Follow-up: `age-documentation-agents-contract-sd7y1.9`.
...
The flywheel health checks exercise capture, retrieval, injection, and citation
mechanics. They do not prove that retrieved context improves task outcomes. Keep
the lifecycle mechanically traceable while measuring usefulness separately.

**Progress:** `scripts/check-flywheel-lifecycle.sh` traces capture → retrieval →
inject → round-trip → citation and soft-fails citation checks on a sparse corpus.
It is not registered in the default Go gate or current CI workflow, and even a
green run proves mechanics rather than task-level uplift.
...
The desired ceilings are CC 20 for `cli/` and CC 18 for `cli/internal/`.

**Progress:** Both declared checks are currently **red** (measured 2026-07-13):
`cli/` has 12 violations with a maximum of 28; `cli/internal/` has 11 with a
maximum of 25. The gate row below remains the target, not a green claim.
Follow-up: `age-documentation-agents-contract-sd7y1.8`.
...
Regeneration and parity checks catch Codex drift before landing. The explicit gate is authoritative; AgentOps does not require a runtime hook while editing.
...
The repository should exercise its experimental knowledge-maintenance surfaces on
a bounded cadence without pretending that file churn proves compounding. The
nightly lane mines recent signal, defragments the local corpus, runs an isolated
dream-cycle proof, measures scenario satisfaction, and publishes evidence for
inspection. End-user scheduling belongs to the operator's chosen substrate.

**Progress:** **Known broken profile mismatch.** `.github/workflows/nightly.yml`
builds the default spine and then invokes `ao mine` and `ao defrag`; those
commands are absent from that build (`defrag` is flywheel-tagged). The lane must
select the archive profile explicitly or stop invoking archived commands before
it counts as implemented. Any resulting artifact is evidence of mechanics, not
...
When a repeated pattern has evidence across sessions, `/postmortem` can identify it and `/skill-builder` can turn the validated practice into a reviewable skill. Promotion remains an explicit human or factory-admission judgment.

**Progress:** Pattern extraction and skill authoring exist as separate live skill routes. Automatic detection, admission, and promotion remain experimental.
...
Durable work intended to land must run through the
[operating loop](docs/architecture/operating-loop.md): BDD-shaped intent →
vertical slices → conflict-free wave when explicitly parallel → bead acceptance
→ evidence and selective ratchet. One-response local or ignored artifacts need
no bead. BDD/Gherkin + DDD + hexagonal architecture + TDD form the behavioral
narrow waist.

A bead-backed change names its [component route](docs/architecture/component-map.md),
carries at least one Given/When/Then acceptance example, slices vertically with
a nameable first failing test, and closes only when acceptance maps to evidence.
Parallel work additionally requires disjoint write scopes.

**Progress:** `scripts/check-loop-shape.sh` resolves the `br` JSONL ledger
first and retains `bd` only as a compatibility fallback. It remains warn-only in
pre-push; `--strict` is reserved for a separately reviewed blocking transition.
Regression coverage: `bash scripts/check-loop-shape.sh --self-test`.
...
**Progress:** The default build ships top-level `ao capabilities` and
`ao robot-docs` (0 → 2 introspection surfaces), plus the `claim` command's
corrective errors. Earlier work on `autodev`, `citation`, `constraint`, and
`plans` does **not** count as shipped on the default surface: `autodev` is
legacy-tagged and the other commands are absent. Remaining work is to audit the
actual default read-side leaves for `--json` fidelity and share the doctor's
extended exit-code vocabulary where it improves corrective guidance.
...
**The destination above, made measurable.** AgentOps reaches it when its own
next epic moves from accepted intent to verified landing without a human at each
transition.

Route milestones (the paving stones, in dependency order — each carries an honest status, not a claim):

1. **Position signal** — the navigator must know where it is. The SDLC provenance ledger is tamper-evident. `ao provenance emit-landed` and `ao provenance emit-verdict`, together with `scripts/pawl-verdict.sh`, record landing and verdict edges. *Status: live; feed health remains measured by the declared gate.*
2. **Resilience / role state-machine** — `ao land <bead>` owns the reviewed
   direct-main happy path. Mechanical redo and breaker transitions are declared;
   autonomous substantive re-scope and recovery remain incomplete.
3. **The gate** — `ao gate check --fast --scope head` supplies deterministic
   evidence and `ao land` requires the exact-candidate review before landing.
   Cross-model review is an assurance posture, not the definition of done.
...
Measured 2026-07-13: 569 verdicts, 548 confirmed, 15 refuted (2.664% refute rate).
Benched-family stall tax was removed from the default
...
|-----|-----------------------|---------------------|---------------------------------------|
| **1. Judgment validation** — agents ship without risk context | Plans skip architecture fit; implementations pass happy path but miss edge cases | `go-vet-clean`, `security-gate`, `contract-compatibility`; `premortem` and `validate` supply semantic judgment | `go-complexity-ceiling` is declared but currently red |
| **2. Durable learning** — solved problems recur | Agents repeat dead-end investigations | `flywheel-proof`, `flywheel-compounding-snapshot` | `flywheel-compounding`, `compile-freshness`, `compile-no-oscillation`, and task-uplift measurement |
| **3. Loop closure** — completed work does not improve the next transition | Sessions end with diffs but no accepted evidence | `goals-validate`, `wiring-closure` | stronger closeout and promoted-learning outcome checks |
...
**Today's enforcement state:** the Current column is wired through current CI,
pre-push, or the Go gate registry. Gap 2 and Gap 3 have blocking checks, but
those checks prove mechanics and structural closure—not that context improves a
future task. Long-cycle and runtime-artifact checks stay in Roadmap until their
execution environment and outcome meaning are reliable.
...
## Scenario satisfaction layer

`ao eval scenario evaluate --all` is the live producer for
`.agents/rpi/scenario-results.json`; nightly automation can invoke it and
`ao goals measure` consumes the results. Missing or unevaluated evidence remains
`unknown`—never an implicit pass or fail. Scenario judgment is observational
unless a declared gate explicitly makes it blocking.
...
|----|-------|--------|-------------|------|
| flywheel-compounding | `bash scripts/check-flywheel-compounding.sh` | 3 | Knowledge flywheel above escape velocity (σρ > δ); requires multi-session citation activity and cannot be moved honestly by a single-session automation run. Treat RED as a long-cycle corpus signal, not a prompt to threshold-game the formula. | long-cycle, corpus-state |
...
| flywheel-proof | `bash scripts/proof-run.sh` | 7 | Cross-session flywheel mechanics produce evidence; this is not proof of task-level uplift. | experimental |
...
| spine-integrity | `timeout 60 bash scripts/check-spine-integrity.sh` | 6 | The 13-skill membrane+bookkeeper spine is intact: each spine skill exists, carries spine:true, passes its validate.sh + schema contract, and leads the SKILLS.md router. |  |
...
| corpus-freshness | `bash scripts/check-corpus-freshness.sh` | 4 | Newest corpus snapshot under `$AGENTOPS_CORPUS_SNAPSHOT_DIR` (default `~/.agentops/corpus-snapshots/`) is within 7 days. Skips cleanly when no snapshots exist. Override: `AGENTOPS_CORPUS_FRESHNESS_SKIP=1`. Snapshot/restore commands are available only from the archive flywheel build (`make build-flywheel`), not the default CLI. |  |
...
| state-path-resolver-coverage | `bash scripts/check-paths-resolver-coverage.sh` | 3 | Tracks executable-code sites that still hardcode `.agents/` paths instead of sourcing the canonical resolver (`lib/ao-paths.sh` / `cli/internal/paths`). Warn-only until its measured baseline justifies a separately reviewed blocking transition. | warn-only |
...
| skill-count-sync | `bash scripts/check-skill-count-sync.sh` | 3 | Skill inventory counts are compared across source directories, active dispositions, and generated/cataloged surfaces; current output is authoritative over hand-written measurements. | warn-only |
