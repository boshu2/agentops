...
| **skills-integrity** | SKILL.md schema + v2 frontmatter, skill-body command/flag refs resolve against the live CLI, skill dependencies resolve, structural heal (`--strict`), headless-runtime skills, manifests valid, no symlinks, local-only `.agents/`, plugin structure | SKILL.md schema/frontmatter violation, a stranded skill-body command/flag ref, an unresolved skill dependency, a symlink, or invalid plugin/manifest structure |
...
| **doctrine-proof** | Flywheel/goals/wiring/corpus/finding-registry/memrl/sovereignty/three-gap proofs PLUS spec-linkage — executable-spec link integrity (`ao goals scenarios --lint`), scenario↔test linkage, AGENTS.md tiered split, docs↔learning references | A failing GOALS/doctrine proof gate, a broken directive↔scenario or scenario↔test link, an AGENTS.md split-contract violation, or a dangling docs↔learning reference |
