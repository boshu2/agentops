...
- [`references/autodev.md`](references/autodev.md) — Autodev: the config/intent layer the loop reads each tick (NOT a loop; the standalone /autodev skill is retired — contract management is absorbed into [`/evolve`](../evolve/SKILL.md), the CLI surface stays `ao autodev`)
