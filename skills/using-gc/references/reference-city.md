...
`overlay/.claude/settings.json` ships ONLY the GC coordination substrate hooks (`gc prime` / `gc handoff` / `gc mail check` / `gc hook`). Per `docs/3.0.md`, AgentOps 3.0 ships zero AgentOps hooks: context flows through the explicit `ao` CLI channel — `ao rpi` runs `ao inject` inside its Research phase and the mayor calls `ao inject --query` directly, so no harness hook is needed.
