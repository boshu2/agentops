# Executable spec for the /postmortem skill — loop closeout (BC3 Loop, Move 7).
# /postmortem wraps completed work: validates it shipped correctly, extracts
...
Feature: Post-mortem closes the loop with evidence and ratcheted learning
  ...
  Scenario: Learnings promote only under the ratchet
    When /postmortem extracts observations
  ...
  Scenario: The cycle produces a durable result artifact
    Then /postmortem writes result.json capturing the closeout verdict + evidence
