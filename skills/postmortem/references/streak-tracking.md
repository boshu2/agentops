...
> Reference for Step 1.5 of the postmortem skill.

## Purpose

Track consecutive days of RPI usage to surface workflow adoption trends in postmortem reports. This is a read-only metric — postmortem does not write streak data.

## Data Source

**File:** `.agents/rpi/rpi-state.json`

This file is written by `/rpi` (the RPI orchestrator) on every session start and phase transition. Postmortem reads it but never writes to it.
...
The streak summary is computed at read time and included in the postmortem report. It is NOT persisted separately.
...
When streak data is available, add this line to the TOP of the postmortem report (before the verdict table):
