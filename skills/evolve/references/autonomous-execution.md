...
Hard stops MUST NOT call `ScheduleWakeup`: KILL/STOP files, dormancy reached, or `--max-cycles` cap hit. When dormancy fires (Step 3 hard-gate), `/evolve` writes `.agents/evolve/DORMANT` and Step 1 short-circuits on subsequent fires before any further tool calls — this is the operational enforcement of the "no ScheduleWakeup on dormancy" rule (see `references/cycle-history.md` for the marker semantics). The terminal-mode `/evolve` loop and the Claude-Code `ScheduleWakeup` loop are duals: both drive Step 1..Step 7 repeatedly, both honor the same kill switches and stop conditions, both persist resume state via `.agents/evolve/session-state.json`.

**`CONTEXT_BUDGET_EXHAUSTED` is NOT a hard stop — it is the ONE handoff that MUST re-arm.** This is the survival mechanism, and it is mechanical, not advisory: on context exhaustion the loop writes the non-sticky `HANDOFF` marker (`references/context-budget.md`) and, **as the last action of the turn, calls `ScheduleWakeup` (delaySeconds≈900–1800)** so a fresh/compacted fire resumes from disk state. Disk is truth — `HANDOFF` + `session-state.json` carry the parked work; Step 1 of the next fire clears `HANDOFF` and continues. **NEVER end the turn, and NEVER `CronDelete`/cancel the loop, for context reasons.** An agent that stops the loop because "this session is full" or "my context is heavy" has made the exact category error this design exists to prevent: one session's context budget is irrelevant to whether the backlog has work. Compaction is something the loop runs *through*, not something that ends it. The only things that end the loop are KILL/STOP, dormancy (empty backlog + dry generators), and `--max-cycles`.
