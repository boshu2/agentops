...
**Checkpoint:** the workflow is invocable (/agy-loop or via `agy -p "/agy-loop <intent>"`) and its steps name a *separate* validator agent — re-read it and confirm the author never appears as the judge.
...
- **Headless crank:** `agy -p "/agy-loop <intent>; STOP after one bead"` from a cron/scheduled task as the AGY turnout's tick.
