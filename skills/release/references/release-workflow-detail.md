...
```bash
./scripts/ci-local-release.sh --ci-blocking  # mandatory local CI parity gate (blocking)
git rev-parse --git-dir           # must be in a git repo
git status --porcelain            # warn if dirty
git branch --show-current         # show current branch
```

`/release` MUST run the repo-root `ci-local-release.sh --ci-blocking` gate first. If it fails, stop the release workflow and report the failing checks.
Once the target version is known, rerun the gate as `./scripts/ci-local-release.sh --release-version <version>` so the artifact manifest matches the final release version. The release artifact gate writes publishable artifacts to `.agents/releases/local-ci/<timestamp>/`, including SBOM files, a full security scan report, and `release-artifacts.json`. Use `./scripts/resolve-release-artifacts.sh <version>` when writing the audit trail.
...
| Local CI parity gate | repo-root `ci-local-release.sh --ci-blocking` | **Block** — must pass before release |
