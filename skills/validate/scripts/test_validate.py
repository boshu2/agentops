...
class ValidateV2Tests(unittest.TestCase):
    ...
    def test_bounded_command_rejects_hard_link_before_spawn(self):
        with tempfile.TemporaryDirectory() as raw:
            base = Path(raw)
            disposable = base / "disposable"
            disposable.mkdir()
            ...
            os.link(outside, disposable / "linked.txt")
            with self.assertRaisesRegex(tool.ContractError, "hard-linked file"):
                tool.run_bounded_command(
                    [sys.executable, "-c", "open('linked.txt', 'w').write('changed')"],
                    cwd=disposable,
                    disposable_root=disposable,
                    authorization_id="test:hard-link",
                    timeout_seconds=1,
                    max_output_bytes=1024,
                    effect="disposable-mutation",
                )
    ...
    def test_bounded_command_rejects_cwd_outside_disposable_root(self):
        with tempfile.TemporaryDirectory() as raw:
            base = Path(raw)
            disposable = base / "disposable"
            outside = base / "outside"
            ...
            with self.assertRaisesRegex(tool.ContractError, "cwd must resolve inside"):
                tool.run_bounded_command(
                    [sys.executable, "-c", f"open({str(marker)!r}, 'w').close()"],
                    cwd=outside,
                    disposable_root=disposable,
                    authorization_id="test:outside-cwd",
                    timeout_seconds=1,
                    max_output_bytes=1024,
                    effect="disposable-mutation",
                )
    ...
    def test_bounded_command_caps_output_and_reaps_process(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertTrue(receipt["process_group_reaped"])
            excerpt_bytes = len(receipt["stdout_excerpt"].encode()) + len(
                receipt["stderr_excerpt"].encode()
            )
            self.assertLessEqual(excerpt_bytes, 4096)
