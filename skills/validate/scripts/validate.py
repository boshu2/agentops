...
def reject_unsafe_links(root: Path) -> None:
    """Reject links that could let a disposable-tree write escape its root."""
        ...
        for name in [*dirnames, *filenames]:
            ...
            if not candidate.is_symlink():
                if candidate.is_file() and info.st_nlink > 1:
                    raise ContractError(
                        f"disposable tree contains a hard-linked file: {candidate}"
                    )
                continue
            try:
...
def run_bounded_command(
    ...
    *,
    ...
    reject_unsafe_links(disposable_root)
    ...
    def drain(name: str, stream: Any) -> None:
        nonlocal excerpt_total, output_total
        while True:
            ...
            with output_lock:
                ...
                remaining = excerpt_limit - excerpt_total
                if remaining > 0:
                    excerpt = chunk[:remaining]
                    excerpts[name].extend(excerpt)
