...
- Start from `.claude/workflows/operating-loop.js`. **Why:** preserving its
...
- **Artifact directory:** `.claude/workflows/` for the runnable script, plus the
  matching `workflows:` row in `docs/contracts/skill-dispositions.yaml`.
- **Filename convention:** `.claude/workflows/<kebab-case-name>.js`; `meta.name`
...
- **Validator command:** run `node --check .claude/workflows/<name>.js`,
