...
sub fail {
    my ($message) = @_;
    print STDERR "agy-native launcher: $message\n";
    exit 125;
}
...
if ($pid == 0) {
    ...
    open STDIN, '<', $stdin_path or do {
        print STDERR "agy-native launcher: open stdin $stdin_path: $!\n";
        exit 125;
    };
    exec { $ARGV[0] } @ARGV or do {
        print STDERR "agy-native launcher: exec $ARGV[0]: $!\n";
        exit 125;
    };
}
...
if (kill 0, -$pid) {
    kill 'KILL', -$pid;
    sleep 0.05;
    if (kill 0, -$pid) { fail("process group $pid survived cleanup"); }
}
if ($signal_exit) {
    print STDERR "agy-native launcher: canceled; process group $pid reaped\n";
    exit $signal_exit;
}
if ($timed_out) {
    print STDERR "agy-native launcher: deadline ${deadline}s expired; process group $pid reaped\n";
    exit 124;
}
