#!/usr/bin/env bash
# validate.sh — self-check for the cc-loop-driver skill (AUTHORING-STANDARD §6/§8).
# Checks: frontmatter completeness, the required section spine + order, the Form-A line
# budget, mandatory Triggers: marker, scripts + spec presence, and that every
# references/ link in SKILL.md resolves on disk. Exit 0 on pass, 1 on any failure.
set -euo pipefail
...
# 'references/' is NOT preceded by another path segment (skip absolute paths like
# ~/dev/agentops/skills/shared/references/... which live outside this skill).
...
# The skill's whole point is no 'claude -p' for workers; it NAMES the ban often.
# Every legit mention sits in a ban context. Flag a 'claude -p' line ONLY when it
# carries no negation/ban token (i.e. it would read as an instruction to use it).
bad=0
...
if [ "$bad" -eq 1 ]; then
  err "a 'claude -p' mention with no ban context appears in SKILL.md (workers must be Task subagents)"
else
  ok "every 'claude -p' mention sits in a ban context"
fi

if [ "$fail" -eq 0 ]; then
  printf '\nPASS: cc-loop-driver SKILL.md meets the authoring standard.\n'
  exit 0
else
