...
Bootstrap may create `.agents/` and root project documents; `ao quick-start --no-beads` deliberately leaves tracker state alone, and Bootstrap does not edit shell rc files or activate hooks. These runbooks cover optional operator-side hygiene when the environment itself looks unhealthy. Read the relevant entry when Bootstrap reports an unexpected skip or when handing a fresh host to a new agent.
...
**When to read:** The host has accumulated many PATH entries, `which ao` or `which br` resolves to a stale or temp-dir copy, shells are slow to start, or a previous installer has left `/tmp/*-install` lines in an rc file. Run before Bootstrap on a long-lived host where the operator has not pruned PATH in a while.
