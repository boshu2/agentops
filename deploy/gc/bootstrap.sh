...
write_marker() {
  local state="$1"
  mkdir -p "$city/.gc"
  python3 - "$marker" "$city" "$rig" "$pack" "$rig_name" "$binding" "$codex_auth" "$max_active_sessions" "$delivery_mode" "$repository_slug" "$telemetry_json" "$state" "$toolchain_json" "$ao_reducer_json" "$city/packs.lock" "$delivery_native_context" "${native_context_digest:-}" "$delivery_evidence_root" <<'PY'
import json
import hashlib
import os
import sys
...
path, city, rig, pack, rig_name, binding, codex_auth, max_active_sessions, delivery_mode, repository_slug, telemetry_json, state, toolchain_json, ao_reducer_json, packs_lock, native_context, native_context_digest, evidence_root = sys.argv[1:]
if not os.path.isfile(packs_lock) or os.path.islink(packs_lock):
...
if state == "ready":
    if not os.path.isfile(native_context) or os.path.islink(native_context):
        raise SystemExit("ready managed city must have a regular native delivery context")
    with open(native_context, "rb") as handle:
        native_raw = handle.read()
    if hashlib.sha256(native_raw).hexdigest() != native_context_digest:
        raise SystemExit("ready managed city native delivery context digest differs")
    payload["delivery"] = {
        "native_context_path": native_context,
        "native_context_digest": native_context_digest,
