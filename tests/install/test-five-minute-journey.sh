...
#   t<180s   Step 4: docs/SKILLS.md router links plan/implement/validate (fact check, not a wording pin)
...
# Step 4: SKILLS router links the plan, implement and validate skills (not /rpi-as-front-door).
# This is a FACT check, not a wording pin: docs/SKILLS.md is generated by
# scripts/generate-skill-mesh.py's router(), whose "Primary entrypoints" table
# links each primary skill's canonical SKILL.md path. Assert the three
# skill-loop links are present rather than pinning any prose phrasing that
# regen is free to reword.
step "Step 4: SKILLS router links plan/implement/validate" \
    "grep -qF 'skills/plan/SKILL.md' docs/SKILLS.md && grep -qF 'skills/implement/SKILL.md' docs/SKILLS.md && grep -qF 'skills/validate/SKILL.md' docs/SKILLS.md && echo OK"
