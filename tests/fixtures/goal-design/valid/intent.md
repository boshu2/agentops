...
Create schema-backed goal-design artifacts for AgentOps.
...
Feature: Goal-design artifacts
  ...
  Scenario: Validate a complete packet
    Given an operator has a goal that should leave their head
    When the goal-design checker validates the packet
    Then both markdown artifacts validate against versioned schemas
    And the driver digest matches the current intent artifact
```
