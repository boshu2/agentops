...
check_contains "$PM_SKILL" '.agents/premortem-checks' "/premortem reads compiled checks first"
check_contains "$PM_SKILL" 'known_risks' "/premortem injects known_risks"
check_contains "$PM_SKILL" 'malformed line -> warn and ignore that line' "/premortem fail-open reader behavior is documented"
check_contains "$PM_SKILL" 'finding-compiler.sh' "/premortem refreshes compiled outputs after registry writes"
check_contains "$VIBE_SKILL" '.agents/premortem-checks' "/vibe reads compiled checks first"
...
check_contains "$POST_MORTEM_SKILL" '.agents/premortem-checks' "/postmortem reads compiled prevention inputs"
check_contains "$POST_MORTEM_SKILL" 'registry.jsonl' "/postmortem writes finding registry"
check_contains "$POST_MORTEM_SKILL" 'atomically update `.agents/findings/registry.jsonl`' "/postmortem uses atomic registry writes"
check_contains "$POST_MORTEM_SKILL" 'finding-compiler.sh' "/postmortem refreshes compiled outputs after registry writes"
