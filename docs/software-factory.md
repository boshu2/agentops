...
| Post-verdict bookkeeping | `/learn` receipt, then an explicit orchestrator decision |
...
| Learning loop | Bind verdict observations and choose the next transition | `/learn`, then `/pattern-mining` or `/operationalize` only for qualified evidence |
...
- **Validation gates** — CI (`.github/workflows/validate.yml`) is the
  ...
  complete.
- **Ratchet checkpoints** — `/learn` persists verdict-bound observations and
  plan impact before the orchestrator chooses the next transition.
