...
## Pattern C — substrate event tailing (NTM + MCP + managed-agents)

Use when: the work runs unattended out of session on an orchestration
substrate (the loop dispatched on the reference orchestration substrate — a scheduled
...
```bash
# Consumer running alongside the orchestration substrate.
