...
2. Run the loop out of session: point the reference substrate at the repo and let
   a controller agent dispatch ready beads to worker panes running `ao rpi`.
...
- Pointed the reference substrate at the repo.
- A controller agent dispatched a ready bead to a worker pane.
