...
- [Forbidden Claims (yaml)](contracts/forbidden-claims.yaml) — Honest-voice lexicon: regex + rationale + allowed_contexts banning CLAIMS of proven/automatic knowledge compounding (unproven — ADR-0004, ADR-0011) and hookless-3.0-violating "session hooks" (docs/3.0.md, ADR-0009) from user-facing CLI strings + seed/template assets; gated by `contract.honest-voice` via `scripts/check-honest-voice.sh` (age-5qjyn)
