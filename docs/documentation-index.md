...
- [AGENTS Operating Contract Behavior](contracts/agents-operating-contract.md) — Nine scenario-level requirements for the compact always-loaded contract; paired isolated judges own meaning and deterministic reconciliation owns artifact identity
- [AGENTS Operating Contract Verdict Template](contracts/agents-operating-contract-verdict.template.json) — Schema-shaped starting packet for one dispatch-bound scenario judgment; paired reconciliation is the only authorizing PASS
...
- [Orchestration Profiles (yaml)](contracts/orchestration-profiles.yaml) — SOT for portable NTM worker and pawl-review role profiles plus structured spawn arguments; drift-gated against `skills/agent-native/SKILL.md` and `skills/pawl-review/SKILL.md` via `scripts/check-orchestration-profiles.sh`
