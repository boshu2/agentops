...
| out-of-session substrate (`ntm` / `ao agent` / MCP) | ORCHESTRATION | Runs whole `ao rpi`/`ao evolve` loops out of session — an **NTM** tmux swarm, **managed-agents** via `ao agent`, or the **MCP** tool surface (`ao mcp serve`). AgentOps owns none of it; it adopts a substrate, the way it adopts `bd`. | Optional (out-of-session only) | Run the loop in-session yourself (`/rpi`, `/evolve`). The substrate adds only always-on orchestration. |
