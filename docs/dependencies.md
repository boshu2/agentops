...
| `gc` (Gas City) | ORCHESTRATION | Out-of-session substrate that runs whole `ao rpi`/`ao evolve` loops: controller/supervisor, Orders, mayor/worker agents. Guided by the [`using-gc`](https://github.com/boshu2/agentops/blob/main/skills/using-gc/SKILL.md) skill — `ao` does NOT wrap it. | Optional (out-of-session only) | Run the loop in-session yourself (`/rpi`, `/evolve`). gc adds only out-of-session orchestration. |
...
| `jq` | UTILITY | Parse `--json` output from `ao`, `bd`, `gc`, and `gh` in scripts and dispatch loops. | Optional | Read JSON manually or use non-JSON output modes; some script automations are unavailable. |
...
| `tmux` | UTILITY | Session multiplexing for streamed/long-running RPI runs and gc's tmux session provider. | Optional | RPI runs in non-tmux modes; gc can use exec/k8s session providers instead. |
...
- **Out-of-session vs in-session.** `gc` is the only orchestration dependency, and it is strictly out-of-session. In-session, an agent runtime + `git` (+ `ao`/`bd` recommended) is the whole stack. See [`using-gc`](https://github.com/boshu2/agentops/blob/main/skills/using-gc/SKILL.md).
