...
- **No always-on daemon, scheduler, or overnight runner shipped by AgentOps.** Those surfaces were deleted; out-of-session orchestration is the substrate's job (reference: NTM + MCP + managed-agents). The in-session loop is the zero-dependency sovereignty floor. **The `ao orchestrate` instrument arm was archived behind a `legacy` build and is absent from the shipping binary** — the out-of-session instrument surface (preflight/verify/tools — deterministic windshield, not a driver) is now exposed through the NTM + MCP Agent Mail substrate (`ao mcp serve` / `ao agent`); ATM, `/goal`, and cron remain swappable driving adapters per [orchestration-ports.md](contracts/orchestration-ports.md#bc6-instrument-layer-out-of-session-waist).
