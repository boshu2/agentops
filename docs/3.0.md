...
- **NTM** (a local tmux agent swarm) — spawns and supervises worker agents that each run the `/rpi` skill over a bead; the operator, or a lead agent, runs `bd ready` and dispatches the next bead.
