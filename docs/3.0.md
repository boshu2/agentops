...
- **NTM** (a local tmux agent swarm) — spawns and supervises worker agents that each run `ao rpi <bead>`; the operator, or a lead agent, runs `bd ready` and dispatches the next bead.
...
In every case the substrate dispatches a whole loop as one invocable unit (`ao rpi <bead>`), `ao inject` / `ao validate --gate` run in the dispatched workdir, and the corpus compounds in `.agents/`. AgentOps never moves its loop, ratchet, or context into the substrate — it adopts orchestration, it does not own it.
...
|---|---|---|
| What it is | A skills + `ao` CLI distribution that adopts NTM + MCP + managed-agents | A sovereign product with its own Rust core |
| Loop encoding | Skills (portable runtime) + the `ao` CLI, dispatched by the substrate | Typed Rust state machines |
| Out-of-session driver | NTM swarm / `ao agent` managed drivers — adopted, not owned | Its own Rust daemon |
...
| Build cost | Low: reuse shipped orchestration (NTM / MCP / managed-agents) | High: re-implement supervision in Rust |
| Sovereignty floor | The in-session loop runs with zero dependencies; always-on adds a substrate | The typed Rust core is the floor |
| When to pick | You want the loop now, with always-on on adopted infrastructure | The typed loop *is* the product, for maximum control and auditability |

The difference in daemon posture is deliberate and is the crux of the deletion decision: **AgentOps has no core of its own to protect** — it owns no orchestration substrate, so it deletes its daemon and adopts one (NTM + MCP + managed-agents) for always-on. **Olympus is a sovereign product** — its typed Rust daemon *is* the product, so it keeps it. AgentOps proves you do not have to hand-roll a daemon to own your factory; Olympus proves you can, when the typed loop is the point. See [ADR-0009](adr/ADR-0009-daemon-deletion-in-session-only.md).
