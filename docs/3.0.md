...
A software factory runs the same loop at every scale: turn declared intent into validated work, then recycle the exhaust as context for the next turn. AgentOps names that loop, runs it **in session**, and makes the exhaust first-class. **Skills** are the portable agent runtime. **rpi** is the inner loop, one research-plan-implement-validate cycle. **evolve** is the outer loop, N rpi cycles toward a goal. **crank** and **swarm** are the in-session agent teams that fan a wave out across worktrees. The loops are **fractal**: the same shape at every layer, run by a human or by a stand-in agent. **Context** is the engineering artifact handed off at every loop edge, and it compounds at every level. The `.agents/` corpus is where it accumulates, and it is the moat. Running that loop **out of session** — always-on, scheduled, queue-driven — is a separate concern that AgentOps delegates to an orchestration substrate.
...
|---|---|---|
| **Orchestration**: when / where / who-supervises / coordination | **Substrate (Gas City is the reference)** | agents (skills via overlay), Orders (cron/event), the bead queue, the mayor and convoys, human on the loop and merge, runtime providers |
| **The in-session loop + the context**: what the agent does, how context compounds | **AgentOps** | rpi (one invocable unit), evolve, crank/swarm, the ratchet rules, skills, `ao inject` / `compile` / `maturity`, the `.agents/` corpus |

**rpi is never re-expressed in the substrate.** Decomposing it into substrate-side steps would duplicate the loop shape (the exact surface-sprawl disease, re-introduced across the seam) and pit the substrate's retry machinery against the ratchet rules. Whoever owns the loop owns its invariants, and AgentOps owns the loop. The substrate dispatches a whole loop as one invocable unit (`ao rpi <bead>`); it never drives the loop's insides.

---

## The Gas City reference City: running the loop out of session

AgentOps ships a **reference Gas City City** — `city.toml` plus `packs/agentops/` — for running the in-session loop out of session on the Gas City PaaS. It is reference, not competition: Gas City drives agents that *use* AgentOps. A long-lived **mayor** agent (team lead: coordinate, merge, notify the human on the loop) and **refinery** worker agents (run the rpi loop) inherit the AgentOps skills via an overlay and run the AgentOps loops.

**Dispatch is mayor-driven, and the reference City says so honestly.** The proven path (validated end-to-end on 2026-05-24, Tiers 1/2/4/5 green) is: a long-lived mayor agent runs `bd ready`, then `gc sling`s the next bead to a refinery worker, which runs `ao rpi <bead>`; cron `exec` Orders handle scheduled maintenance (`ao compile`, `ao maturity --scan`). The skills overlay lands, `ao inject`/`ao validate --gate` run in the dispatched workdir, and the corpus compounds.

**Order-level autonomous dispatch is a known Gas City maturity gap, not a turnkey AgentOps feature.** A cooldown Order that pulls the next ready bead and binds it to a dispatch formula on its own cannot work as shipped: Gas City Orders have no per-fire variable-binding mechanism, so the formula's required `issue` var has no value to bind. This is an **upstream Gas City evolution** the operator contributes to (tracked at `soc-5jwah`); the reference City labels it as such and does **not** claim turnkey order-auto-dispatch. The honest capability today is mayor-driven dispatch; order-auto-dispatch grows with Gas City. Evidence and the full per-tier breakdown: [`.agents/discovery/2026-05-24-gvkj6-e2e-proof.md`](https://github.com/boshu2/agentops/blob/main/.agents/discovery/2026-05-24-gvkj6-e2e-proof.md).
...
| Axis | AgentOps-on-Gas-City (PaaS-native) | Mount Olympus (full-custom Rust) |
|---|---|---|
| What it is | A Gas City distribution: the reference `city.toml` + `packs/agentops` | A sovereign product with its own Rust core |
| Loop encoding | Skills (portable runtime) + the `ao` CLI, dispatched by Gas City | Typed Rust state machines |
| Out-of-session driver | Gas City (mayor-driven dispatch; order-auto-dispatch is an upstream GC gap) | Its own Rust daemon, with Gas City as one optional adapter |
| Daemon | **None** — AgentOps deleted its daemon; always-on opts into Gas City | **Kept** — the daemon is the product; a sovereign core to protect |
| Build cost | Low: reuse a shipped supervision substrate | High: re-implement supervision in Rust |
| Sovereignty floor | The in-session loop runs with zero dependencies; always-on adds Gas City | The typed Rust core is the floor |
| When to pick | You want the loop now, with always-on on proven infrastructure | The typed loop *is* the product, for maximum control and auditability |

The difference in daemon posture is deliberate and is the crux of the deletion decision: **AgentOps has no core of its own to protect** — it is a Gas City reference config, so it deletes its daemon and opts into Gas City for always-on. **Olympus is a sovereign product** — its typed Rust daemon *is* the product, so it keeps it. AgentOps proves you do not have to hand-roll a daemon to own your factory; Olympus proves you can, when the typed loop is the point. See [ADR-0009](adr/ADR-0009-daemon-deletion-in-session-only.md).
...
- **No always-on daemon, scheduler, or overnight runner shipped by AgentOps.** Those surfaces were deleted; out-of-session orchestration is the substrate's job (Gas City is the reference substrate). The in-session loop is the zero-dependency sovereignty floor.
