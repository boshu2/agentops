...
- **The default install works with zero hooks.** Workflow is guided by skills plus the `ao` CLI. The repository owns delivery and may choose direct push, PRs, external CI, or a bounded deterministic hook for local or cloud agents. AgentOps does not ship or require one.
...
Validation ends when `/validate` emits the immutable evidence-bound verdict and
`/learn` emits the bookkeeping and plan-impact receipt. Git delivery starts
after that boundary. A repository may direct-push, open a PR, or use hosted CI.
None of those choices change whether AgentOps has proved the work.
...
- [x] **Hookless by default:** AgentOps 3.0 ships zero hooks. Workflow is guided by skills plus the `ao` CLI; each repository owns any hook, CI, PR, or direct-push policy it chooses.
