...
| 5 | **Humans move up the abstraction ladder** | "From Operator to Architect… move up the abstraction ladder… review Designs, Intent, Policies, not lines." | "Validate BOTH ENDS" + BDD-as-plan in [`operating-loop.md`](../architecture/operating-loop.md) move 1; coherent-arc review in `CLAUDE.md` | ✅ encoded |
...
| 7 | **Intent/spec as the plan, authored before code** | "Co-authoring and approving detailed specifications with AI before code generation." | "Behavior is the plan" — [`operating-loop.md`](../architecture/operating-loop.md), `skills/discovery/SKILL.md`, `skills/brainstorm/SKILL.md` | ✅ encoded |
| 8 | **Progressive authorization / autonomy levels** | **L0–L4 ladder**, gated on statistically-significant success vs Golden data; "Red Button" override. | In-session floor → NTM background sessions → bounded `autodev`: [`canonical-loop-model.md`](../architecture/canonical-loop-model.md), `skills/agent-native/SKILL.md` | ⚠️ partial → `ag-wrom` |
...
| 10 | **Transparency / Chain-of-Thought / immutable provenance** | CoT in real-time UIs; deterministic actuation traces persisted (Spanner). | Append-only provenance ledger (`agentops-sdlc-provenance.v1`) — `CLAUDE.md` provenance section, evidence capture under the ratchet | ✅ encoded |
