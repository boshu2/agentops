...
| Codex parity audit fails after editing a spine skill (e.g. `skills/validate/SKILL.md`) | The Codex side at `skills-codex/validate/SKILL.md` drifted | Mirror the change on the Codex side, then run `scripts/regen-codex-hashes.sh`. Non-spine (ambient/experimental) skills are `excluded` and carry no twin — no mirror needed. |
