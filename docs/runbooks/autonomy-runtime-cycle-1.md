...
1. Pull latest `main` and sync beads (`git fetch --prune origin && git switch main && git reset --hard origin/main`; then `BEADS_DIR="$(ao beads dir)" br sync` if used in this clone).
...
3. Capture failing artifacts and create follow-up bead(s) with references (`BEADS_DIR="$(ao beads dir)" br create "..." --body "..."`).
