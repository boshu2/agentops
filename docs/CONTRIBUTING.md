...
- Every internal link must resolve. `mkdocs build --strict` fails on unresolved relative links; `tests/docs/validate-links.sh` catches the same class without a Python toolchain. `--check` tolerates only the enumerated, intentional cross-references in `tests/docs/mkdocs-strict-allowlist.txt` (docs pages linking to real repo artifacts outside the site) — see [the strict-allowlist policy](../tests/docs/mkdocs-strict-allowlist.txt); any *other* strict warning fails the build, so fix the link rather than silencing it.
