...
- **Sync embedded copies after editing source**: After editing `lib/chain-parser.sh`, `skills/standards/references/`, or `skills/curate/scripts/`, run `cd cli && make sync-hooks` (it copies those into `cli/embedded/`). Tests and builds use the embedded copies, not the source files. (The target name is legacy — AgentOps 3.0 is hookless; it embeds skills/lib, not runtime hooks.)
