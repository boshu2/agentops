...
| `ao rpi` · `ao evolve` command surface | The in-session operating loop + the `/rpi` skill (one turn over the loop) | The `ao rpi` command lane was removed at `f61c5f0e7` (3.2); the loop itself is now the navigation path, not a CLI verb. Restore the old command surface with `AGENTOPS_LEGACY=1 make build`. |
