...
const gapsOk = await codexPass(
  `Read ${DIR}/behaviors.md (Gherkin behaviors). You are an INDEPENDENT cross-family reviewer (you did not write them). List the TOP 15 MISSING behaviors — edge cases, error paths, and "done" conditions the author under-specified or skipped — RANKED by how badly an implementer could ship broken while satisfying the written scenarios. Concrete and specific; 15 max, not an exhaustive dump. Write to ${DIR}/behaviors-codex-gaps.md.`,
