...
unattended, dispatch the operating loop on the NTM + MCP Agent Mail substrate
(a tmux swarm of workers coordinating through Agent Mail locks/inboxes);
AgentOps ships no daemon or scheduler of its own (see [ADR-0009](../adr/ADR-0009-daemon-deletion-in-session-only.md)).
