...
AgentOps' current differentiation is the combination of repo-native `.agents` memory, scored promotion, citations, validation gates, and the in-session `/dream` maintenance cycle (run out of session on the reference substrate: NTM + MCP + managed-agents). Competitors usually have one or two of those pieces, not the full closed loop.
...
Dream is a separate contract. The `/dream` skill runs a private local maintenance loop that ingests knowledge, reduces it through defrag/dedup/compile/prune, measures health, and commits only improved staged artifacts. It never mutates source code, never invokes RPI, and emits a morning packet. It runs in session; to run it out of session, unattended, dispatch it on the reference substrate (NTM + MCP + managed-agents — AgentOps ships no daemon or scheduler of its own; see [ADR-0009](../adr/ADR-0009-daemon-deletion-in-session-only.md)).
...
|---|---|---|
| Dreaming | `/dream` skill (in session) + reference-substrate out-of-session dispatch (NTM / MCP / managed-agents) + nightly CI dream-cycle proof job | Shipped |
