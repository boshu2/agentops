...
Dream is a separate contract. The `/dream` skill runs a private local maintenance loop that ingests knowledge, reduces it through defrag/dedup/compile/prune, measures health, and commits only improved staged artifacts. It never mutates source code, never invokes RPI, and emits a morning packet. It runs in session; to run it out of session, unattended, dispatch it on the NTM + MCP Agent Mail substrate (AgentOps ships no daemon or scheduler of its own — see [ADR-0009](../adr/ADR-0009-daemon-deletion-in-session-only.md)).
...
|---|---|---|
| Dreaming | `/dream` skill (in session) + NTM + Agent Mail out-of-session dispatch + nightly CI dream-cycle proof job | Shipped |
