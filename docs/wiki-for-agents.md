...
**Always-on compounding.** Run defrag, evolve, compile, and dream against the corpus out of session by handing the loop to an orchestration substrate — an NTM swarm, MCP-aware harness, or managed-agents driver can run the same `ao` commands while you sleep. A SaaS wiki cannot get smarter on its own.
