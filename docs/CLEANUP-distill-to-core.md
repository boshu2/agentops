...
2. **Relocate orchestration → MTO** *(deprecate-stub, don't hard-delete)* — `autodev`, `batch_*`, `cron*`, `pool*`, `operator`, `orchestrate`, `rpi serve` become stubs that point to MTO. Acceptance: relocate scenario.
