...
6. Only then point at the optional out-of-session compounding lane, which runs the same loop on an orchestration substrate (NTM + MCP + managed-agents is the reference) — AgentOps itself ships no daemon or scheduler.
...
| Optional out-of-session lane | 5 min | A substrate driver (NTM / MCP / managed-agents) is configured to run the loop unattended. |
...
delegated to a substrate, and AgentOps adopts a reference one: **NTM + MCP +
managed-agents**.

On the reference substrate, a long-lived **lead agent** in an NTM swarm runs `bd ready` and
dispatches the next bead to a worker that runs `ao rpi <bead>`; MCP exposes the `ao`
tool surface to any harness; managed-agents provide hosted unattended execution via
`ao agent`. Scheduled maintenance (`ao compile`, `ao maturity --scan`) runs on cron.
The agents run the same loop you just ran by hand. See the [AgentOps 3.0 north star](3.0.md)
for the in-session / out-of-session split and the substrate trio.
...
| NTM / MCP / `ao agent` config | The adopted substrate for the optional out-of-session lane, only after first trust exists. |
