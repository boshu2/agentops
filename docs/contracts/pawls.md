...
## Warm pawl-service — REMOVED

The optional standing warm service (`ao pawl up` / `scripts/pawl.sh` over tmux/NTM panes)
was removed. The membrane front door is cold-only: `ao pawl review` → `pawl-review.sh`
(spawning `codex exec`) → `pawl-verdict.sh` → pre-push enforcement. See the strict
two-family cold quorum below for the portable multi-model door.
...
diversity requirement. **`ao verify --strict`** (or the `strict: true` key in `.aoverify.yaml` →
