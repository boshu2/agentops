...
Contract for `.agents/rpi/next-work.jsonl` — the carry-forward queue that feeds
evidence-backed follow-up producers into RPI selectors and related pre-flight
checks. Postmortem performs causal analysis only; it does not own this queue.
...
| `items` | array of Item | yes | Harvested follow-up work; may be empty when a producer finds nothing actionable |
