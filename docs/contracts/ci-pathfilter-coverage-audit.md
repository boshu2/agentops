...
| `doctrine-proof` → Validate AGENTS.md detail-owner contract | `AGENTS.md`, `docs/agent-workflow-reference.md`, `docs/CI-CD.md`, `docs/contracts/codex-skill-api.md`, `docs/contracts/repo-execution-profile.md` | `ci, docs, shell` | **GAP → FIXED** (this PR) |
...
### Gap 1 — AGENTS detail owners uncovered (the direct #634 class)

`scripts/validate-agents-split.sh` validates `AGENTS.md` **and** the four detail owners (`docs/agent-workflow-reference.md`, `docs/CI-CD.md`, `docs/contracts/codex-skill-api.md`, `docs/contracts/repo-execution-profile.md`) for existence and bidirectional links. The gate step triggers on `docs || ci || shell`. `AGENTS.md` is covered by the `contracts` filter (post-#638) but `contracts` was not in the trigger, and the four owner docs were covered by **no** filter except `markdown` (which no gate consumes). So an edit to `docs/agent-workflow-reference.md` alone — e.g. a future docs-thinning that breaks the bidirectional link — would skip the very gate that enforces the contract. This is the identical mechanism that disabled the security canary in `bbfd278e`.

**Fix:** add the four detail-owner paths to the `contracts` filter (they are operator-contract source, sibling to `AGENTS.md` which is already there), and add `|| needs.changes.outputs.contracts == 'true'` to the agents-split gate step's trigger.
