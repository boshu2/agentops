...
| `goal-design` | persistent | skills, operators, tests | goal-design-packet | Schema-backed goal-design packets (`intent.md` + `driver.md`) under `.agents/goal-design/<slug>/`; the deterministic checker is the only Goal Design gate, while Premortem later judges the exact final plan |
...
| `spawn` | rolling | skills, operators | spawn-evidence | Per-session worker metadata/evidence written through `AgentWorker`; NTM robot state and optional Agent Mail coordination are adapter evidence, not a second task store |
