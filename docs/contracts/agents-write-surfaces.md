...
| `spawn` | rolling | skills, operators | spawn-evidence | Per-session spawn metadata JSON (e.g. `.agents/spawn/<session>.json`) written by ATM spawn checklists; read by `ao orchestrate verify` strong evidence tier |
| `state` | persistent | cli | state-memory | Durable admitted state findings under `.agents/state/findings/`, written only by `ao state admit` after schema, non-self-review, freshness, leak, and path-confinement checks; read by `ao state verify` and the `state.verify` gate |
