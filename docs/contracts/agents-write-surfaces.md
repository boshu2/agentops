...
| `pawl-verdicts` | persistent | scripts, skills | decision-record | Machine-checkable cross-family pawl verdicts (schema `schemas/pawl-verdict.v1.schema.json`) written by `/pre-land-refuters` via `scripts/pawl-verdict.sh write` and read by `scripts/reconcile-pr.sh` to gate merge-to-main (fail-closed: no CONFIRMED verdict, no merge) |
