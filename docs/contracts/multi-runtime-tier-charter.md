...
Claude Code and Codex remain first-class plugin installs. Cursor, OpenCode,
Gemini CLI/Antigravity, Pi, Grok Build and OpenClaw reach AgentOps through the
external npx Skills installer; Grok Bot takes the same `SKILL.md` files through
its own skill settings; the Gemini package is retired. Missing live evidence
does not retire a promised journey.
...
| Gemini CLI / Antigravity | npx Skills installer: `-a gemini-cli` or `-a antigravity`. The `agentops-core-gemini` package and its generated image are retired. Contributor source linking detects `~/.gemini/skills`. | Repository catalog/frontmatter checks cover the input, as for other npx consumers. | Installer success is not Gemini or Antigravity load proof. Each claimed host journey, optional dependency and migration needs its own native evidence. |
| Pi and portable source-link consumers | npx Skills installer: `-a pi` (user scope `~/.pi/agent/skills`). For contributors, the existing [destination resolver](https://github.com/boshu2/agentops/blob/main/cli/internal/skillsapp/roots.go) always includes `~/.agents/skills` and detects Pi via `~/.pi/agent`, linking into `~/.pi/agent/skills`; `--dest` selects one explicit root for other consumers. | [Source-link tests](https://github.com/boshu2/agentops/blob/main/cli/internal/skillsapp/link_test.go) and [unlink tests](https://github.com/boshu2/agentops/blob/main/cli/internal/skillsapp/unlink_test.go). | These are existing filesystem consumers, not a new claim of live host qualification. Host discovery and behavior remain separately unproven until exercised. |
| `npx skills` consumers | External Skills installer reads this repository's skills; users pick agents interactively or name them with `-a`. Its agent table includes `cursor`, `opencode`, `gemini-cli`, `antigravity`, `pi`, `grok` (Grok Build) and `openclaw`. It does not install the runtime plugin, AO, roles or hooks. | Repository catalog/frontmatter checks cover the input. The external installer's current `--help` owns its selection, link/copy and update semantics. | Record installer version, selected agents and installed source. External installation success does not establish any host's I or E tier. |
| Grok Bot | No installer target. The same `SKILL.md` folders are added through Grok Bot's own skill settings. | None. | I and E unproven. |

Contributor source-link discovery is defined by the resolver, not by detecting
running applications: existing `~/.claude`, `~/.codex`, `~/.gemini` and `~/.cursor`
directories select their `skills/` roots, and an existing `~/.pi/agent` directory
selects `~/.pi/agent/skills`. `--dest` overrides this fan-out.
...
from absence of an AgentOps automation lane. Observations of npx-installed
hosts (Cursor, OpenCode, Gemini CLI/Antigravity, Pi, Grok Build, OpenClaw) and
Grok Bot likewise use their native authorized hosts.
