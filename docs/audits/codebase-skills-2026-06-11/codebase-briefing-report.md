...
**Tooling signals**
- CI: 11 workflows; `validate.yml` = 2,020 lines (tiered T0 ≤30s / T1 ≤5min / T2 ≤15min, all required). Local wall: `scripts/pre-push-gate.sh` = 2,210 lines, with Go orchestrator `ao gate check` opt-in at partial parity (12/79 checks native per `MEMORY.md`/ag-qidx).
...
| 2 | **Dual-gate drift (bash wall vs Go orchestrator).** The 2,210-line `pre-push-gate.sh` is the default wall while `ao gate check` is opt-in at 12/79 native parity; two implementations of 79 checks will diverge. | **High** | Finish the ag-3n71 Phase-B migration to a single Go-owned gate (or formally freeze the bash gate and generate it); add a parity test that diffs check inventories between the two. |
