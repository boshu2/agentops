...
[AgentOps 3.0 north star](../3.0.md)). On the reference substrate (NTM + MCP + managed-agents), a
...
| Dream compounding run | `/dream "<goal>"` (foreground, in session) | Reads `summary.json` / `summary.md` from the run dir | Private local compounding in a watchable session. Running it unattended is out-of-session orchestration, delegated to a substrate (the substrate is the reference) — AgentOps ships no daemon or scheduler |
