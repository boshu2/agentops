...
4. Fill `Recurrence before` from the catch count at fix time. Leave `Recurrence after` to be filled by the *next* postmortem that runs triage — that is the measurement.
...
|---|---|---|---|---|---|
| 2026-07-08 | **Tracker-shape fail-open** — code/skills assume `br`'s JSON shape; on `bd` a field is absent (e.g. `show` omits `dependents[]`, and omits an empty `description`), so an audit/consumer silently sees zero and passes. Caught on `age-f07z` (multi-family panel; a single reviewer missed it). Recurred in a local bd-backed postmortem when the Codex closure auditor reported zero children and `collection_failed=true`. | judgment | Rule: *tracker-agnostic code must enumerate children via the supported tracker adapter (never `show \| jq .dependents[]`), and treat absent fields as null.* The Codex postmortem script still violates the rule and needs parity with the corrected producer surface. | 1 → 1 additional recurrence (2026-07-10) | escalate: binding failed |
