...
> **Note:** This document describes the legacy hookless fallback for Codex versions prior to v0.115.0. Starting with Codex v0.115.0 (March 2026), native hooks are supported and installed by `scripts/install-codex-plugin.sh`. Current Codex native hooks (see `hooks/codex-hooks.json`) cover `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, and `Stop`, but not `SessionEnd`. Because Codex `Stop` fires per-turn rather than at session teardown, it is not a `SessionEnd` substitute, so the hookless fallback and explicit closeout path still matter when transcript-driven maintenance is required.
