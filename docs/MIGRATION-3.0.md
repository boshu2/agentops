...
The CLI gc-bridge glue is severed. `ao rpi` keeps its non-gc backends (`auto`/`direct`/`stream`/`tmux`); `runtime=gc` is no longer a valid mode. A substrate now dispatches whole loops (`ao rpi`) as one unit rather than driving the phased engine through a bridge.
...
- **The reference out-of-session substrate** — AgentOps adopts **NTM** + **MCP** (`ao mcp serve`, shipped) + **managed-agents** (`ao agent`) to run the loop out of session. None of it is AgentOps-owned; the substrate dispatches a whole `ao rpi`/`ao evolve` loop as one unit and never drives its insides.
