...
5. **Right-reason first RED.** Before any integration or production-checker edit, `tests/go_cli_integration_baseline.bats` contains exactly one valid Bats declaration `@test "integration baseline rejects stale ancestry" {`. Its body runs the integration checker and fails only because `git merge-base --is-ancestor origin/main HEAD` returns nonzero while `origin/main` is not an ancestor. A missing script, missing Git/Bats tool, syntax error, setup error, no-match filter, or any other harness failure is not RED.
