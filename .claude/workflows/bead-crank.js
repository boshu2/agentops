...
// ===========================================================================
// PHASE 3+4 — FAN/CRANK + RECONCILE, interleaved per wave.
...
const runImplementer = (beadId, baseSha) => () =>
  agent(implementerPrompt(beadId, baseSha), { label: `crank:${beadId}`, phase: 'Fan-Crank', schema: AGENT_RETURN_SCHEMA })
...
for (const wave of planGraph.waves) {
  ...
  if (wave.parallelizable && ids.length > 1) {
    ...
    log(`Fan-Crank wave [${ids.join(',')}]: PARALLEL fan-out (cap ${PARALLEL_CAP}).`)
    ...
    for (const id of ids) {
      log(`Fan-Crank link ${id}: SEQUENTIAL${lastMergedSha ? ` (base ${lastMergedSha})` : ' (base origin/main)'}.`)
      ...
      if (led.confirmed_merged) {
        const tip = await agent(
          ...
          { label: `base-sha:${id}`, phase: 'Fan-Crank', schema: { type: 'object', required: ['sha'], properties: { sha: { type: 'string' } } } })
...
log(`Fan-Crank+Reconcile done · ${merged}/${reconcileLedger.length} confirmed-merged${blocked ? ` · ${blocked} blocked` : ''}`)
