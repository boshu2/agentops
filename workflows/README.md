...
Before a batch, one additional cheap agent runs an exact Node command through Bash to resolve target paths with `realpath` and obtain existing files' device/inode identities with `stat`. It reads no file contents. The workflow rejects aliases (including symlinks and hardlinks), missing or invalid metadata, and failed probes before any writer starts. Missing targets resolve through the nearest existing ancestor; dangling symlinks and non-file targets fail the probe. When either target is absent, names that match after NFC normalization and lowercasing are conservatively rejected on every host: for example, `New.js` and `new.js` can become the same file on macOS. Use distinct portable names. Node must be available to that agent. Writers then run sequentially, one per item, in the shared working tree. A single-item call needs no cross-item identity check.
