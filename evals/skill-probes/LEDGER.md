...
| `premortem` | `premortem-plan-shape-t2` | 2026-08-26 | BEHAVIORAL | Tier-2 seeded plan review, canonical-skill, gpt-5.6-luna at effort `low`, N=2 — **DIRECTIONAL, not statistical**. Control 0/2, treatment 1/2: both control reps flagged 6 and 7 of 8 steps (armchair pessimism, out of the declared band `[1,4]`); the passing treatment rep returned 3 findings including both seeded steps. Headroom pre-screen `skill.probe-headroom` = **SEPARATED**, control aced 0 effort levels — a group whose control rate is 0.00 at the measured level cannot reach the two-level saturation bar. scorecard: `docs/evals/scorecards/2026-08-26/premortem-plan-shape-t2-low.json`; fixture manifest: `evals/skill-probes/premortem-plan-shape-t2/fixtures-low-2026-08-26/fixture-set.json`. Second effort level not run (budget); producer dispatched with an isolated `CODEX_HOME` so neither arm carried an ambient skill corpus — see `evals/skill-probes/RUNBOOK.md`. |
