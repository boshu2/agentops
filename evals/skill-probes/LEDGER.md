...
| `premortem` | `premortem-plan-shape-t2` | 2026-09-03 | INERT | scorecard: `docs/evals/scorecards/2026-09-03/premortem-plan-shape-t2-low.json`; sealed dispatch (one removed-on-exit run directory reset per rep; reads denied under the real HOME, the real temp and cache roots, the checkout, the git common directory, the real CODEX_HOME, and every skill root; network denied except to a harness-owned CONNECT proxy whose host set, port, and empty socket list the verifier pins; web search disabled in a generated config the verifier compares to its renderer; auth copied; the seal outermost with the timeout inside it and the rep's process group counted before it is reaped; capture-contract v3 seal block whose profile the verifier rebuilds and digest-checks); producer gpt-5.6-luna effort low; control 0/2, treatment 0/2; every command any rep ran is in the transcripts (at this level, none), none touched a denied path, the sibling-read and skill-read traps fired on none, and no rep was refused egress; headroom SEPARATED; N=2, directional. Fifth capture of this probe on 2026-09-03 (fixture set `fixtures-low-2026-09-03-final`); the four superseded ones, each under a seal a judge then broke and deleted with their scorecards, read at low: treatment 1/1, 0/2, 0/1, 1/2 usable. |
| `premortem` | `premortem-plan-shape-t2` | 2026-09-03 | INERT | scorecard: `docs/evals/scorecards/2026-09-03/premortem-plan-shape-t2-xhigh.json`; same seal and producer at effort xhigh (`fixtures-xhigh-2026-09-03-final`); control 1/2, treatment 1/2; every command any rep ran is in the transcripts (one: the control-1 rep ran `rg --files ... && rg -n ...` over its empty workspace and got exit 1), none touched a denied path, the sibling-read and skill-read traps fired on none, and no rep was refused egress; headroom SEPARATED; N=2, directional; the first control rep of any capture to put the marks in band. Superseded xhigh captures read 0/2, 2/2, 0/2, 0/2. Across all five captures the treatment arm showed the marks in band in 5 of 18 usable reps and the control arm in 1 of 20; that aggregate spans five harness versions and is disclosure, not a ledger row. |
...
`fixtures-{low,xhigh}-2026-09-03-final`, the fifth capture, after the fourth pass
