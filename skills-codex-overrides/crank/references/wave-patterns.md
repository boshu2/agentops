...
Wave 1: br ready → [issue-1, issue-2, issue-3]
        ...
        $swarm → spawns 3 fresh-context agents
               ...
               DONE      DONE      BLOCKED
                               ...
                               (retry in next wave)
        ↓
        br update --status closed for completed
...
2. **Load acceptance criteria** for all issues closed in this wave:
   ...
   br show <issue-id>  # extract ACCEPTANCE CRITERIA section
   ```
...
6. **Gate on verdict:**
   ...
   br update <epic-id> --append-notes "CRANK_ACCEPT: wave=$wave verdict=<PASS|WARN|FAIL> at $(date -Iseconds)"
   ```
