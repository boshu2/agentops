...
Coding agents declare "done" on code that is still wrong. AgentOps is the **operating loop** that turns declared intent into validated code with proof: shape behavior (Gherkin), implement against a failing acceptance test, then bind an independent membrane verdict (a check by a model or test that did not write the code) to **that** contract. **No verdict = not done.** Skills are the front door on the agent you already use (Claude Code, Codex, Cursor, OpenCode).
...
**Live skills from a clone (optional).** Already have the repo checked out? `ao skills link` *symlinks* its skills into the live tier of **every agent runtime you have installed** (`~/.claude/skills`, `~/.codex/skills`, `~/.gemini/skills` (AGY), `~/.cursor/skills`, `~/.pi/skills`). Unlike the copy-based installers above (which snapshot skills at install time), local edits and every `git pull` take effect next session with no re-copy:
...
Opt-in live/edit-in-place tier for people working from a clone; the installers above stay the copy-based path for everyone else. Never copies or clobbers: existing non-AgentOps skills (e.g. other marketplaces) are reported as conflicts and left untouched. `--dest <dir>` targets one specific dir instead.

Installs hookless. Hard requirements: an agent runtime and `git`; everything else degrades gracefully. Dependencies: [docs/dependencies.md](docs/dependencies.md) · Day-2 ops (update, backup, recovery): [docs/install-day2-ops.md](docs/install-day2-ops.md).

Verify: open your agent and type `/plan`. It should resolve as a skill (restart Codex first).
...
Validation completion and Git delivery are separate. After the verdict, use your repository's own direct-push, PR, merge, and CI policy.
...
# Experimental (corpus compounding still measured; see The honest version):
...
**Proven:** independent verification that records a verdict, and a durable, tamper-evident record of it. A change isn't done until something that didn't write it checks it against the declared acceptance behavior, and that verdict is bound into the provenance ledger. No verdict, not done. Receipts: [membrane receipts](docs/evidence/membrane-receipts.md) (every number from the verdict ledger).

**Still measuring:** whether the accumulated corpus makes the next session measurably better ([ADR-0004](docs/adr/ADR-0004-corpus-moat-unproven-position-on-the-system.md), [ADR-0011](docs/adr/ADR-0011-escape-corpus-compounding-unproven-structural-starvation.md)).

AgentOps proves the work. Your agent still writes the code; the cross-checks cost tokens. `.agents/` is plain markdown your agents keep up as they go. When labs ship their own version, that folder comes with you: your repo, Apache-2.0.
