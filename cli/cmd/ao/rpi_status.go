...
// Search-root discovery migrated to internal/rpi (search_roots.go) so the
// next-work-proof reader and keeper commands stay self-contained after the
// ao rpi command surface is removed (ADR-0009 teardown, age-3pdt). These thin
// aliases keep existing call sites and tests unchanged.
var (
	collectSearchRoots      = cliRPI.CollectSearchRoots
	tryAddSearchRoot        = cliRPI.TryAddSearchRoot
	normalizeSearchRootPath = cliRPI.NormalizeSearchRootPath
)
