...
func TestHelper2_truncateID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.id, func(t *testing.T) {
			if got := truncateID(tt.id, tt.max); got != tt.want {
...
// Ensure os import is used (evictionCitationStatus tests need time but not os directly).
