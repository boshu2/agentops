...
// TestCapabilitiesSixCommandSliceReportsRealContracts pins declared==observed
...
func TestCapabilitiesSixCommandSliceReportsRealContracts(t *testing.T) {
	cases := []struct {
		path    string
		args    string
		output  string
		...
		{"ao gate check", "no-args", "text", "filesystem,process,environment,clock", map[string]string{"0": "success", "1": "failure", "2": "usage"}},
...
// TestVersionExitCodesObservedMatchDeclared verifies the version command's
...
func TestVersionExitCodesObservedMatchDeclared(t *testing.T) {
...
// TestCapabilitiesUncontractedCommandsAreHonestlyUnknown asserts inspect.go no
...
func TestCapabilitiesUncontractedCommandsAreHonestlyUnknown(t *testing.T) {
