...
func TestCapabilitiesEnvironmentInputsMatchExecutableOwners(t *testing.T) {
	...
	for input := range want {
		description, ok := capabilitiesEnvVars[input]
		if !ok {
	...
	// The projection must not carry stale names unsupported by the default build.
	var stale []string
	for input := range capabilitiesEnvVars {
		if !want[input] {
...
func TestCapabilitiesCommandExitCodesMatchConstants(t *testing.T) {
	...
	verify := capabilitiesCommandExitCodes["verify"]
	for _, code := range []string{"0", "1", "2", "3", "4", "5"} {
		if verify[code] == "" {
...
// TestCapabilitiesRecursiveCommandsPublishExecutableHoldExits is the
// differentiating oracle for F-12: the recursive commands[] contract must
// publish every typed verdict an executable front door can return. A malformed
// committed verify policy HOLDs before review, while the shared pawl engine
// propagates strict HOLD/UNAVAILABLE verbatim; both are exit 5.
func TestCapabilitiesRecursiveCommandsPublishExecutableHoldExits(t *testing.T) {
	...
	// The pawl front door exposes the same typed code: its strict engine returns
	// 5 for HOLD/UNAVAILABLE and runPawlReview propagates it verbatim.
