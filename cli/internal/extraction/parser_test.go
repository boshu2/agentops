...
func createTestMessage(content string) domain.TranscriptMessage {
...
func TestExtractor_Extract_DeduplicatesByType(t *testing.T) {
	...
	decisionCount := 0
	for _, r := range results {
		if r.Type == domain.KnowledgeTypeDecision {
			decisionCount++
		}
	}
	if decisionCount != 1 {
...
func TestExtractBest_HigherScoreLaterInSlice(t *testing.T) {
	...
	for range 100 {
		...
		if best.Type == domain.KnowledgeTypeSolution {
			sawHighScore = true
		}
	}
	if !sawHighScore {
...
func parseCodexEdgeLine(t *testing.T, p *Parser, jsonl string) *domain.TranscriptMessage {
	t.Helper()
