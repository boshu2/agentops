// Package extraction provides streaming JSONL parsing for Claude and Codex transcripts.
package extraction
...
func (p *Parser) Parse(r io.Reader) (*ParseResult, error) {
	result := &ParseResult{
		Messages: make([]domain.TranscriptMessage, 0),
	}
...
func (p *Parser) parseContentBlocks(blocks []any) (string, []domain.ToolCall) {
	var content string
...
func (p *Parser) classifyBlock(blockMap map[string]any) (string, *domain.ToolCall) {
...
func (p *Parser) parseLine(line []byte, lineNum int) (*domain.TranscriptMessage, error) {
	var raw rawMessage
...
func (p *Parser) parseClaudeMessage(raw rawMessage, lineNum int) *domain.TranscriptMessage {
...
func (p *Parser) parseClaudeToolUse(raw rawMessage, lineNum int) *domain.TranscriptMessage {
	var tools []domain.ToolCall
	if raw.ToolName != "" {
		tools = append(tools, domain.ToolCall{
			Name:  raw.ToolName,
			Input: raw.ToolInput,
		})
	}
...
func (p *Parser) parseClaudeToolResult(raw rawMessage, lineNum int) *domain.TranscriptMessage {
...
func (p *Parser) parseCodexSessionMeta(raw rawMessage, lineNum int) (*domain.TranscriptMessage, error) {
...
func (p *Parser) parseCodexEvent(raw rawMessage, lineNum int) (*domain.TranscriptMessage, error) {
...
func (p *Parser) parseCodexResponseItem(raw rawMessage, lineNum int) (*domain.TranscriptMessage, error) {
...
func (p *Parser) extractMessageContent(rawContent any, msg *domain.TranscriptMessage) {
...
func (p *Parser) parseToolUse(block map[string]any) *domain.ToolCall {
...
func (p *Parser) parseToolResult(block map[string]any) *domain.ToolCall {
	toolCall := &domain.ToolCall{
		Name: msgTypeToolResult,
	}
...
func (p *Parser) ParseChannel(r io.Reader) (<-chan domain.TranscriptMessage, <-chan error) {
	msgCh := make(chan domain.TranscriptMessage, 100)
...
func (p *Parser) processChannelLine(line []byte, lineNum int, msgCh chan<- domain.TranscriptMessage, errCh chan<- error) bool {
	if len(line) == 0 {
		return true
	}
	msg, err := p.parseLine(line, lineNum)
...
func (p *Parser) channelScanner(r io.Reader, msgCh chan<- domain.TranscriptMessage, errCh chan<- error) {
	if err := readJSONLLines(r, func(line []byte, lineNum int) error {
