...
func NewContextCommand(useCases ContextUseCases) *cobra.Command {
	...
	var field string
	command := &cobra.Command{Use: "context", Short: "Resolve an authorized context route without writing", Args: cobra.NoArgs,
...
The bundle may be external or the canonical direct <consumer-root>/.context.
Staging and evidence must remain outside Git and disjoint from consumer and bundle.
Plain reading of selected cleared project Markdown needs neither BD nor AO.
...
it never creates replacement configuration, knowledge or work.`,
	...
	for _, item := range []struct {
		...
		{"consumer-root", &req.ConsumerRoot, "Existing consumer checkout; only direct .context may hold the bundle"}, {"native-directory", &req.NativeDirectory, "Explicit native BD source directory"},
		{"bundle-root", &req.Overrides.BundleRoot, "Selected existing external bundle or direct consumer .context"}, {"evidence-root", &req.Overrides.EvidenceRoot, "Selected existing protected non-Git evidence"}, {"staging-root", &req.Overrides.StagingRoot, "Selected existing protected non-Git staging"},
