...
import (
	"bufio"
	"crypto/sha1" //nolint:gosec // git object IDs are SHA-1 by definition; not a security primitive here.
	"crypto/sha256"
	"encoding/hex"
	"encoding/json"
	"fmt"
...
func GitBlobOID(content []byte) string {
	h := sha1.New() //nolint:gosec // git blob IDs are SHA-1; matching git, not securing anything.
