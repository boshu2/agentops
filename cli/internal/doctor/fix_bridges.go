...
// The bridges subsystem detects failures in the OpenClaw consumer surface.
// Per the Phase 2 analysis, most failure modes are detect-only — the doctor
// must never install binaries, start or kill processes, or rewrite activation
// files. The single partial fixer is fm-bridges-openclaw-snapshot-stale, whose
// torn-`latest.json` sub-case is safely reconstructible from an intact
// versioned snapshot.
