...
- **Multi-phase work:** Prefer explicit skill sessions with scoped beads, validation, and provenance. `ao rpi`/`ao evolve` remain compatibility surfaces, but background-agent work is moving to NTM-supervised Claude/Codex skill sessions.
- **Before spawning workers:** Verify no file overlap across the wave. File collisions are the #1 swarm failure mode.
- **Before proposing new capability:** Check existing skills, `.github/workflows/validate.yml`, `GOALS.md`, and relevant `ao` command help first.
- **Out-of-session substrate — NTM background agents, not an in-CLI mode.** AgentOps ships no daemon or scheduler; always-on work is handled by **NTM** tmux swarms of Claude/Codex sessions coordinated through **mcp-agent-mail** and equipped with **MCP** / `ao` tools. These agents run skill-guided sessions, not deprecated `ao rpi`/`ao evolve` wrappers. `ao` does NOT own or wrap NTM; it supplies bootstrap, profile, context, validation, and provenance support. See [docs/3.0.md](docs/3.0.md) and [docs/dependencies.md](docs/dependencies.md).
