...
- **Multi-phase / multi-agent work (opt-in escalation, NOT the default):** single-agent-first is the default (`docs/3.0.md`, operating-loop.md; doctrine ag-single-agent-first). Multi-agent **escalates to** the orchestration substrate — **NTM** (tmux agent swarms) + **MCP Agent Mail** (locks / messaging / inboxes) + the `continuity-loop` renewal spine, under `.agents/agent-constitution.md` — only when single-agent is insufficient (contention / durability), never as a forced substrate. The in-session `ao rpi` loop is **retired as the live workflow** — do NOT route new work through it. (Its Go lane is load-bearing legacy, not dead code — see "Legacy RPI lane" below; the command still compiles but is not how work is driven.)
