...
- **Out-of-session substrate — NTM + MCP + managed-agents, not an in-CLI mode.** AgentOps ships no daemon or scheduler; always-on work is delegated to a swappable substrate: an **NTM** tmux swarm, **MCP** via `ao mcp serve` (shipped), or **managed-agents** via `ao agent`. Each dispatches a whole `ao rpi`/`ao evolve` loop as one unit; `ao` does NOT own or wrap a substrate. See [docs/3.0.md](docs/3.0.md) and [docs/dependencies.md](docs/dependencies.md).
