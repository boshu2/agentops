...
#   2. each metadata-derived bundled slug resolves to
...
[[ -f "$CATALOG" ]] || { printf 'FAIL: missing metadata-derived skill catalog\n' >&2; exit 1; }

fail() { printf 'FAIL: %s\n' "$*" >&2; exit 1; }
pass() { printf 'PASS: %s\n' "$*"; }

command -v jq >/dev/null || fail "jq is required"
mapfile -t all_skills < <(jq -r '.skills[].name' "$CATALOG")
...
catalog_count="$(jq -r '.skill_count' "$CATALOG")"
[[ "$expected_count" == "$catalog_count" ]] || fail "bundle count $expected_count differs from catalog count $catalog_count"
pass "all $expected_count metadata-derived slugs resolve to skills/<slug>/SKILL.md and match source byte-for-byte"
...
printf 'OK: Gemini/AGY image bundle is valid (%s metadata-derived skills, direct+wrapped, zero conversion)\n' "$expected_count"
