...
#
# NOTE: parity_only Codex twins (skills-codex/<name>/{SKILL.md,prompt.md}) are now
# AUTO-generated from source by scripts/codex-sync.sh (run as a step below) — a
# pointer body naming skills/<name>/SKILL.md as the source of truth, so they
# cannot drift. Only bespoke (hand-authored) Codex profiles are manual; mark a
# skill bespoke in skills-codex-overrides/catalog.json to opt out of generation.
set -euo pipefail
...
if [[ "$MODE" == "regen" ]]; then
  ...
  echo "Note: parity_only skills-codex/<name>/ twins are now AUTO-generated by"
  echo "  scripts/codex-sync.sh (pointer to source). Only a bespoke (hand-authored)"
  echo "  Codex profile needs manual authoring — add it to skills-codex-overrides/catalog.json."
