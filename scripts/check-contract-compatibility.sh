...
cleanup() {
  rm -f "$CATALOG_CONTRACTS_TMP" "$ALLOWLIST_ENTRIES_TMP" "$ALLOWLIST_PATHS_TMP"
}
trap cleanup EXIT
...
if [[ -f "$CATALOG" ]]; then
  for contract in "$CONTRACTS_DIR"/*; do
    ...
    if grep -Fxq "$rel_contract" "$CATALOG_CONTRACTS_TMP" 2>/dev/null; then
  ...
  while IFS= read -r entry; do
    ...
    if grep -Fxq "$allowlisted_path" "$CATALOG_CONTRACTS_TMP" 2>/dev/null; then
