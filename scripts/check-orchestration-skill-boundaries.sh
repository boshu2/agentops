...
for skill in "${active[@]}"; do
  [ -f "$skill" ] || {
    echo "canonical orchestration skill is missing: $skill" >&2
    exit 1
  }
done
...
rg -q 'single local agent pays no factory coordination cost' skills/agent-native/SKILL.md
rg -q 'External NTM documentation' skills/ntm/SKILL.md
rg -q 'self-describing `am`' skills/agent-mail/SKILL.md
