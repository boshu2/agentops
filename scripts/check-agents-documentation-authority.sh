...
for arg in "$@"; do
  case "$arg" in
    ...
    *)
      echo "check-agents-documentation-authority: unknown argument: $arg" >&2
      exit 2
      ;;
  esac
done
...
case "$phase" in
  ...
  *)
    echo "check-agents-documentation-authority: invalid phase: $phase" >&2
    exit 2
    ;;
esac
...
try:
    ...
    print(
        "check-agents-documentation-authority: PyYAML is required",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
try:
    ...
    print(
        f"check-agents-documentation-authority: cannot read manifest: {exc}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
if not isinstance(data, dict):
    print(
        "ERROR: manifest must be a mapping",
        file=sys.stderr,
    )
    raise SystemExit(1)
...
for index, record in enumerate(documents):
    ...
    reject_unknown(
        references,
        {"phase_policy", "consumers", "include_globs"},
        f"{path}.references",
    )
    ...
    if not isinstance(consumers, list) or any(
        not isinstance(item, str) or not item for item in consumers
    ):
...
actual_root = sorted(
    ...
    if item.name.endswith(".md") and (item.is_file() or item.is_symlink())
)
declared_root = sorted(seen_paths)
missing = sorted(set(actual_root) - set(declared_root))
required_absent = sorted(
    path
    for path in set(declared_root) - set(actual_root)
...
try:
    ...
    print(
        f"check-agents-documentation-authority: cannot read git index: {exc}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
for path, record in records.items():
    ...
    if not isinstance(references, dict):
        continue
    raw_consumers = references.get("consumers", [])
    consumers = (
        [item for item in raw_consumers if isinstance(item, str) and item]
        if isinstance(raw_consumers, list)
        else []
    )
    raw_include_globs = references.get("include_globs")
    include_globs = (
        raw_include_globs
        if isinstance(raw_include_globs, list)
        and all(isinstance(item, str) and item for item in raw_include_globs)
        else None
    )
    ...
    raw_policies = references.get("phase_policy", {})
    policy = raw_policies.get(phase) if isinstance(raw_policies, dict) else None
    if policy == "ignore":
        continue
    live: list[str] = []
...
if errors:
    ...
    print(
        f"check-agents-documentation-authority: FAIL ({len(errors)} errors)",
        file=sys.stderr,
    )
    raise SystemExit(1)

print(
    "check-agents-documentation-authority: PASS "
